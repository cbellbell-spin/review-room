...
class ProofEditorImpl implements ProofEditor {
  ...
  private buildReviewPanelHost(): ReviewPanelHost {
    return {
      ...
      addSelectionComment: (selection, text) => this.addReviewRoomSelectionComment(selection, text),
      persistReviewMarks: () => this.persistReviewRoomReviewMarks(),
      replyToComment: (commentId, text) => Boolean(this.markReply(commentId, getCurrentActor(), text)),
